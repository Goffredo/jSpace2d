...
public abstract class GameRenderable {
	...
	public void setTriangles(ArrayList<Triangle> triangles) {
		...
		//trianglesBuffer = ByteBuffer.allocateDirect(triangles.size()*4*9).asFloatBuffer();
		//normalsBuffer = ByteBuffer.allocateDirect(triangles.size()*4*9).asFloatBuffer();
		//fill Buffers
		...
		/*
		verticesArray = new Float[triangles.size()*9];
		...
		for(int i = 0; i<triangles.size(); i++){
			...
			for(int j = 0; j<3; j++){
				verticesArray[i*9+j*3] = faceVertices[j].x;
				verticesArray[i*9+j*3+1] = faceVertices[j].y;
				verticesArray[i*9+j*3+2] = faceVertices[j].z;
				
				normalsArray[i*9+j*3] = faceNormals[j].x;
				normalsArray[i*9+j*3+1] = faceNormals[j].y;
				normalsArray[i*9+j*3+2] = faceNormals[j].z;
